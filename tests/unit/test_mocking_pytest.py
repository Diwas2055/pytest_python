...
def test_mock_patch_function_with_dice(mocker):
    ...
    mocker.patch(
        ...
        "src.mocking.random.randint",
        return_value=4,
    )
    # Testing if run_dice returns "Lucky".
    assert mocking.run_dice() == "Lucky"
    assert mocking.random.randint.call_count == 1
    assert mocking.random.randint.call_once_with(1, 6)
    assert mocking.random.randint.call_with(1, 6)
    ...
    assert mocking.run_dice() == "Unlucky"
    assert mocking.random.randint.call_count == 2
...
def test_create_example(mocker):
    ...
    # Mocking the object
    repository_mock = mocker.patch.object(mocking.ExampleRepositoryInterface, "create")
